...
namespace GK_Projekt2
{
    public class Drawer
    {
        ...
        public static void FillPolygons(DirectBitmap image, Triangles triangles)
        {
            ...
            if (mainForm.Method1)
            {
                if (mainForm.SetCoefficient)
                {
                    ...
                    {
                        FillPolygon(_triangles[i].ActiveEdges, image);
                    });
                }
                else
                {
                    Parallel.For(0, _triangles.Length, i =>
                    {
                        FillPolygonSetCo(_triangles[i].ActiveEdges, image, _triangles[i].Kd, _triangles[i].Ks, _triangles[i].m);
                    });
                }
            }
            else if(mainForm.Method2)
            {
                if (mainForm.SetCoefficient)
                {
                    Parallel.For(0, _triangles.Length, i =>
                    {
                        FillPolygonMethod2(_triangles[i], image);
                    });
                }
                else
                {
        ...
        public static void FillPolygonSetCo(List<ActiveEdge> edges, DirectBitmap image, double kd, double ks, double m)
        {
        ...
        public static void FillPolygon(List<ActiveEdge> edges, DirectBitmap image)
        {
            Vector3D N = mainForm.defaultN;
            Vector3D L = mainForm.defaultL;
            Vector3D V = new Vector3D(0, 0, 1);
        ...
        private static void FillPolygonMethod2(Triangle t, DirectBitmap image)
        {
            ...
            if (mainForm.ConstObjectColor)
            {
                ...
                VectorRA = 2 * VectorNA - VectorLA;
                VectorRA.Normalize();

                VectorRB = 2 * VectorNB - VectorLB;
                VectorRB.Normalize();

                VectorRC = 2 * VectorNC - VectorLC;
                VectorRC.Normalize();
